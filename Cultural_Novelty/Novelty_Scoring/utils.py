...
def data_analysis(data_dict, ref=True, col_name='Train_Variations', gen_type='dist'):
    
    recettes = []
    if col_name=='LLM_gen':
        if gen_type == 'dist':
            indexes = []
            key_list = list(data_dict[col_name]['same_country_novelty'].keys())
            for key in key_list:
                recette = data_dict[col_name]['same_country_novelty'][key]['Instructions']
            ...
            cult_keys = list(data_dict[col_name]['cult_novelty'].keys())
            for cult in cult_keys:
                key_list = list(data_dict[col_name]['cult_novelty'][cult].keys())
                for key in key_list:
                    recette = data_dict[col_name]['cult_novelty'][cult][key]['Instructions']
                    recette = re.sub(r'\\u00b0', ' degree', recette)  # Use re.sub to replace the Unicode degree symbol with the word "degree"
                    recette = re.sub(r'(\d+)\\', r'\1 inch', recette) ## Replace any sequnce of Number// by Numberinch
                    recettes.append(recette)
        elif gen_type == 'same':
            indexes = []
            if 'same_country_novelty' in data_dict[col_name]:
                same_key_list = list(data_dict[col_name]['same_country_novelty'].keys())
                for key in same_key_list:  #Here we are in same_country key that directly access novelty generated answers
                    recette = data_dict[col_name]['same_country_novelty'][key]['Instructions']
                    recette = re.sub(r'\\u00b0', ' degree', recette)  # Use re.sub to replace the Unicode degree symbol with the word "degree"
                    recette = re.sub(r'(\d+)\\', r'\1 inch', recette) ## Replace any sequnce of Number// by Numberinch
                    recettes.append(recette)

            key_countries = list(data_dict[col_name]['variation_novelty'].keys())  ## here we have country_origin + variations
            for key_c in key_countries:
                novel_keys = list(data_dict[col_name]['variation_novelty'][key_c].keys())
                for key_no in novel_keys:
                    if key_no != 'country':  ## Not making the 'country' key from the country_origin data
                        recette = data_dict[col_name]['variation_novelty'][key_c][key_no]['Instructions']
                        recette = re.sub(r'\\u00b0', ' degree', recette)  # Use re.sub to replace the Unicode degree symbol with the word "degree"
                        recette = re.sub(r'(\d+)\\', r'\1 inch', recette) ## Replace any sequnce of Number// by Numberinch
                        recettes.append(recette)
    else:
...
def ingr_cleaning(ingredient_list):
...
def text_cleaning(recipe_list, lemma="True", authrorized_pos= ['PROPN', 'PRON', 'ADJ', 'ADV', 'NOUN', 'NUM', 'VERB']):
